...
def capture_report_log_errors(job, demux_out, augmet_run_obj):
    name = augmet_run_obj.name
    age = augmet_run_obj.age
    gender = augmet_run_obj.gender
...
def return_passed_samples(job, demux_out, sample_dict, p1):
    pass_list = ["pass"]
    fail_list = ["fail"]
    job.log(f"lili={p1.name}")
    return pass_list, fail_list

def start(job):
    job.log(f"Entered into start function")
    try:
        p1 = AugmetRun(job, "Aman", "2000-07-10")
        p1.age = "2000-07-19"  # This will work
        job.log(f"updated age to {p1.age}")
        run_demultiplex = job.addChildJobFn(submit_demultiplex, p1)
        demux_out = run_demultiplex.rv(0)
        sample_dict = run_demultiplex.rv(1)
        run_demultiplex.addChildJobFn(capture_report_log_errors, demux_out, p1)
...
if __name__ == "__main__":
    ...
    Job.Runner.startToil(parent_job, options)
    # parent_job = Job.wrapJobFn(start)
